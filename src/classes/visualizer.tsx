...
type event<State = any, Downstream = any> =
  | merge_event<State>
  | operation_event<State, Downstream>;
...
export default class visualizer<State = any, Downstream = any> {
  ...
  public new_value(
    ...
    return this.crdt.value(state, showMetadata);
  }
  ...
  public remove_operation(replica_id: number, op_time: number) {
    ...
    this.event_list[replica_id] = this.event_list[replica_id].filter(
      o => !(o.type == "operation" && o.timestamp == op_time)
    );
  }

  public move_operation(
    replica_id: number,
    op_Current_time: number,
    op_New_time: number
  ) {
    this.resultsCalculated = false;
  ...
  private getVectorclock(replica: number, time: number): Vectorclock {
    ...
    for (let i = 0; i < this.event_list[replica].length; i++) {
      ...
      if (e.type == "merge") {
        vc = vc
          .with(replica, i + 1)
          .merge(this.getVectorclock(e.from_replica, e.from_time_stamp));
      } else {
