...
class Update extends React.Component<Props, State> {
  ...
  handleDragEnd = (e: any) => {
    ...
    if (e.target.getStage().getPointerPosition().x > 700) {
      ...
      this.props.graph.setState({});
      if (
        typeof this.props.fromReplica === "number" &&
        typeof this.props.toReplica === "number" &&
        this.props.fromReplica !== this.props.toReplica
      )
        this.props.visualizer.remove_merge(
          this.props.fromReplica,
          this.props.fromX,
          this.props.toReplica,
          this.props.toX
        );
    } else {
