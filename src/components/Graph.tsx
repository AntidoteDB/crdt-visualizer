...
class Graph extends React.Component<Props, States> {
      ...
      >
        ...
        >
          ...
          {this.state.updates.map((update, index) => (
            <Group key={update.props.fromX}>{update.render()}</Group>
          ))}
          <TooltipForState
            x={this.state.MouseX}
            y={this.state.MouseY}
            ...
            state={
              "State:" +
              this.props.visualizer!.new_value(
                this.state.Actual_Replica_ID,
                this.state.MouseX,
                true
              )
            }
        ...
        <Layer>
          ...
          <TooltipForState
            ...
            text={"State:"}
            visible={
              this.state.isDragging || this.state.Operation_dragging
                ? false
                : true
            }
          />
  ...
  onMouseDown = (e: any) => {
    ...
    this.setState({
      isDrawing: true,
      fromX: e.target.getStage().getPointerPosition().x,
  ...
  onmouseup = (e: any) => {
    ...
    if (
      this.state.fromReplica != this.state.toReplica &&
      ...
      this.props.visualizer.add_merge(
        this.state.fromReplica,
        this.state.fromX,
        this.state.toReplica,
        this.state.toX
      );
    } else {
      this.setState({ isDrawing: false });
    ...
    if (this.state.isDrawing) {
      ...
      if (
        e.target.getStage().getPointerPosition().x >=
          this.replica_start_point_x &&
        e.target.getStage().getPointerPosition().x <= this.replica_End_point_x
      ) {
        this.setState({
          MouseX: e.target.getStage().getPointerPosition().x,
  ...
  setToReplica(e: any) {
    ...
    if (
      e.target.getStage().getPointerPosition().x < this.replica_start_point_x
    ) {
      return;
    }
    ...
    for (var i = 0; i < this.replicas_y_positions.length; i++) {
      if (
        ...
        this.setState({
          fromReplica: i,
          toReplica: "no replica",
          fromY: this.replicas_y_positions[i]
        });
        break;
      } else {
        this.setState({
  ...
  getReplicaId() {
    ...
    for (var i = 0; i < this.replicas_y_positions.length; i++) {
      if (
        Math.abs(this.state.MouseY - this.replicas_y_positions[i]) < variance
      ) {
